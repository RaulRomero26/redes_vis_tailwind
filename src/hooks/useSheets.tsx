...
export const useSheets = () => {
    ...
    const [_currentSheet, _setCurrentSheet] = useState(1);
    ...
    const deleteSheet = async (id: number) => {
        ...
        localStorage.removeItem(`storedPositionsH_${id}`);
    };
   ...
   useEffect(() => {
    if (network && selectedSheet !== prevSheet) {
        const handleStoreNetwork = async () => {
            ...
            const options = JSON.parse(localStorage.getItem(`opciones_${prevSheet}`) || '{}');
            localStorage.setItem(`opciones_${prevSheet}`, JSON.stringify(options)); // Guardar opciones actualizadas
        };
        handleStoreNetwork();
        setPrevSheet(selectedSheet); // Actualizar prevSheet después de guardar
    }
   ...
   useEffect(() => {
    if (network) {
        const loadNetwork = async () => {
            ...
            const storedOptions = localStorage.getItem(`opciones_${selectedSheet}`);
